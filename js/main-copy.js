...
if (losses[i] >= 60) {
  ...
  console.log("This team is not so bad.")
}


// loops through all the teams putting the data into the table
// i=i+1 = i++ (could also be written this way)
...
for (var w=0; w < teams.length; w++) {
  if (w == tempLeader) {
    ...
    tableBuilder += '<tr><td>${teams[w]}<strong></td><td>${wins[w]}</td><td>${losses[w]</td></tr>`;
  }
}
...
// use temp in front of variables to indicate they are temporary
var tempHighWins = 0;
var tempLeader = 0;
...
for (var i=0; i<teams.length; i++){
  if(wins[i] >= tempHighWins){
    tempHighWins = wins[i];
    tempLeader = i;
    console.log(tempHighWins[i]);
    console.log(tempLeader[i]);
  }
}
